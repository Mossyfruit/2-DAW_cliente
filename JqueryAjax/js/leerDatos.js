...
function leerDatos() {
    ...
    let peticionAjax = clienteAjax(url, datos, metodo); // respuesta en formato JSON
    ...
    peticionAjax.done(function (data) {
        for (let row of data) { // RECORRO CADA UNO DE LOS 10 REGISTROS
            var linea = document.createElement("tr"), // creo una fila
                ...
                campoEdad = document.createElement("td"), // creo una celda para la edad
            ...
            campoEdad.innerHTML = row[1]; // escribo la edad contenida en el array
            ...
            linea.appendChild(campoEdad); // añado a la línea la celda con la clave
